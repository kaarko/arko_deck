#!/usr/bin/env node
/**
 * Export script — captures each slide as a hi-res screenshot then assembles into a PDF.
 *
 * Usage:
 *   node export-arko-sales-deck.js [output.pdf]
 */
...
const HTML_FILE    = path.resolve(__dirname, '../ArkoSalesDeck.html');
...
function resolveOutFile() {
  ...
  const existing = fs.readdirSync(EXPORTS_DIR)
    .map(f => f.match(/^ArkoSalesDeck(\d+)\.pdf$/))
    .filter(Boolean)
    .map(m => parseInt(m[1], 10));
  const next = existing.length ? Math.max(...existing) + 1 : 1;
  return path.join(EXPORTS_DIR, `ArkoSalesDeck${next}.pdf`);
}
